...
namespace YimMenu::Hooks
{
	...
	static bool ScanExplosionEvent(CExplosionEvent& event, Player player)
	{
		...
		{
			g_log.send("INFO", std::format("blocked invalid interior crash from {}", player.GetName()));
			return true;
		}
		...
		if (event.m_IsInvisible)
		{
			...
			{
				// Camera shake
				g_log.send("WARNING", std::format("Blocked EXPLOSION_EVENT from {} since m_CameraShake > 0.0f", player.GetName()));
				return true;
			}
			...
			{
				// Ragdoll
				g_log.send("WARNING", std::format("Blocked EXPLOSION_EVENT from {} since m_DamageScale > 0.0f", player.GetName()));
				return true;
			}
		}
	...
	bool ShouldAllowNetEvent(Player player, uint16_t event_id, rage::datBitBuffer& buffer)
	{
		...
		case rage::netGameEvent::Type::WEAPON_DAMAGE_EVENT:
		{
			...
			{
				g_log.send("WARNING", std::format("Blocked WEAPON_DAMAGE_EVENT from {} with m_WeaponType == WEAPON_TRANQUILIZER", player.GetName()));
				return false;
			}
		...
		case rage::netGameEvent::Type::CHANGE_RADIO_STATION_EVENT:
		{
			...
			{
				// Vehicle takeover
				g_log.send("WARNING", std::format("Blocked CHANGE_RADIO_STATION_EVENT on our local vehicle from {}", player.GetName()));
				return false;
			}
		...
		case rage::netGameEvent::Type::DOOR_BREAK_EVENT:
		{
			...
			g_log.send("WARNING", std::format("Blocked DOOR_BREAK_EVENT from {}", player.GetName()));
			return false;
		}
		...
		case rage::netGameEvent::Type::SCRIPT_WORLD_STATE_EVENT:
		{
			...
			if (type == CScriptWorldStateEvent::Type::PopGroupOverride)
			{
				...
				if (pop_group == 0 && (percentage == 0 || percentage == 103))
				{
					...
					g_log.send("WARNING", std::format("Blocked SCRIPT_WORLD_STATE_EVENT of type PopGroupOverride with invalid params from {}", player.GetName()));
					return false;
				}
			}
			...
			{
				// Stop traffic
				g_log.send("WARNING", std::format("Blocked SCRIPT_WORLD_STATE_EVENT of type PopMultiplierArea from {}", player.GetName()));
				return false;
			}
		...
		case rage::netGameEvent::Type::SCRIPT_ENTITY_STATE_CHANGE_EVENT:
		{
			...
			if (type == CScriptEntityStateChangeEvent::Type::SettingOfTaskVehicleTempAction)
			{
				...
				if (action >= 15 && action <= 18)
				{
					...
					g_log.send("WARNING", std::format("Blocked SCRIPT_ENTITY_STATE_CHANGE_EVENT of type SettingOfTaskVehicleTempAction with invalid params from {}", player.GetName()));
					return false;
				}
				...
				{
					// Ped takeover
					g_log.send("WARNING", std::format("Blocked SCRIPT_ENTITY_STATE_CHANGE_EVENT of type SettingOfTaskVehicleTempAction on our local ped from {}", player.GetName()));
					return false;
				}
				...
				{
					// Vehicle takeover
					g_log.send("WARNING", std::format("Blocked SCRIPT_ENTITY_STATE_CHANGE_EVENT of type SettingOfTaskVehicleTempAction on our local vehicle from {}", player.GetName()));
					return false;
				}
			}
			else if (type == CScriptEntityStateChangeEvent::Type::SetVehicleLockState)
			{
				...
				{
					g_log.send("WARNING", std::format("Blocked SCRIPT_ENTITY_STATE_CHANGE_EVENT of type SetVehicleLockState from {} on our local vehicle", player.GetName()));
					return false;
				}
			}
			else if (type == CScriptEntityStateChangeEvent::Type::SetVehicleExclusiveDriver)
			{
				...
				{
					// Vehicle kick
					g_log.send("WARNING", std::format("Blocked SCRIPT_ENTITY_STATE_CHANGE_EVENT of type SetVehicleExclusiveDriver from {} on our local vehicle", player.GetName()));
					return false;
				}
			}
			else if (type == CScriptEntityStateChangeEvent::Type::SetPedFacialIdleAnimOverride)
			{
				if (Self::GetPed().GetNetworkObjectId() == entity)
				{
					g_log.send("WARNING", std::format("Blocked SCRIPT_ENTITY_STATE_CHANGE_EVENT of type SetPedFacialIdleAnimOverride from {} on our local player", player.GetName()));
					return false;
				}
			}
			else if (type > CScriptEntityStateChangeEvent::Type::SetVehicleExclusiveDriver)
		...
		case rage::netGameEvent::Type::NETWORK_PLAY_SOUND_EVENT:
		{
			...
			{
				g_log.send("WARNING", std::format("Blocked NETWORK_PLAY_SOUND_EVENT from {} with IsEntity: {}, RefHash: {:X}, SoundHash: {:X}, SoundId: {}, ScriptHash: {:X}", player.GetName(), event.m_IsEntity, event.m_RefHash, event.m_SoundHash, event.m_SoundId, event.m_ScriptId.m_Hash));
		...
		case rage::netGameEvent::Type::REPORT_MYSELF_EVENT:
		{
			...
			{
				g_log.send("WARNING", std::format("Blocked REPORT_MYSELF_EVENT from {} with type {} since we're in a cheater-only session", player.GetName(), event.m_0x30));
				return false;
			}
		...
		case rage::netGameEvent::Type::ACTIVATE_VEHICLE_SPECIAL_ABILITY_EVENT:
		{
			...
			{
				// Vehicle takeover
				g_log.send("WARNING", std::format("Blocked ACTIVATE_VEHICLE_SPECIAL_ABILITY_EVENT on our local vehicle from {}", player.GetName()));
				return false;
			}
		...
		case rage::netGameEvent::Type::RAGDOLL_REQUEST_EVENT:
		{
			...
			if (Self::GetPed().GetNetworkObjectId() == event.m_PedToRagdoll)
			{
				...
				g_log.send("WARNING", "Blocked RAGDOLL_REQUEST_EVENT on our local ped from {}", player.GetName());
				return false;
			}
